...
| retune the castle dressing | `Config.Structure.Buttress` / `Torch` — pitch, size, proudness, light | the castle family: every post inside a solid run with its clearance, torch `range` under Roblox's silent 60 clamp, the bracket above the machine line, the flame under the trim, the leash margin with `proud` counted, and the part budget at every land state |
