...
```bash
python3 tools/verify.py            # fifteen passes, and it regenerates build/. Run before every commit.
...
| move the belt | `Config.Layout.BeltSideX/BeltFrontZ/BeltBackZ/BeltInnerX` — the two ground paths mirror from these | `inPlot`, the mirror assertions, machine spacing, trigger dwell, drop budget, the run-offs into the vault |
| change the plot's walls | `Config.Structure` (`WallHeight`, `Course`, `Openings`), `GateCentre/GateWidth`; the builder is `buildWallRing` in `src/server/tycoon/Installers.lua`; the PURCHASES are `Config.StructureButtons` | the doorway span, the gateway vs the belt, openings staying on the centre span, the course fit, the part budget at every land state |
| retune the castle dressing | `Config.Structure.Buttress` / `Torch` — pitch, size, proudness, overshoot, light | the castle family: the buttress row anchored on every corner an opening does not swallow, posts out of openings, height inside `(WallHeight, WallHeight + overshoot]`, torch `range` under Roblox's silent 60 clamp, the bracket above the machine line, the leash margin with `proud` counted, and the part budget at every land state |
| retune how walls and gates break | `Config.Structure.Health` — level and masonry tier are the two axes | monotone health in level AND tier, wall out-lasts gate over the whole grid, no one-swing gate, a maxed stone gate breaks inside 90 s, repair inside the raid warning |
...
| change the belt's guard rails | `Config.Layout.BeltGuard` — at a bend a rail runs flush to the neighbour's surface edge, derived from `BeltWidth` | a leg's rail may not overlap another leg's running surface — revert the verifier's flush model to full-length rails and watch the deleted rails' bug come back |
...
| retune the tower, or add a floor archetype | `Config.Tower`, `Config.TowerArchetypes` + a case in `TowerService.spawnFloor`/`stepRun` | the tower family: the deck's shape over twenty seeds, fighting-beats-waiting, the run fits a sitting, the world ceiling with runs standing — plus `tower_spec` |
| disclose a new surface, or move an arrival | a row in `Config.Disclosure`; gate the panel on `HUD.disclosed(id)` | the disclosure family: ids unique, `after` resolves, ≤3 always-on rows, help under 160 chars — plus `disclosure_spec`'s high-water |
| add a daily objective or a hint | a row in `Config.Objectives.Pool` / `Config.Hints` | the objectives family: known stats only, the richest-day bound, the seeded draw — plus `objective_spec`'s baseline |
| add or rename a rank | a row in `Config.Tiers` | the tier family: strictly climbing from zero, unique names, the top rank at any count |
...
| reorder the plot shell, or add a masonry tier | move or add the ROW in `Config.StructureButtons` plus its `Config.Structure.Tiers` row — a PARALLEL track gated on `dropper1` | the same chain assertion, `gates` needs `walls`, each tier needs the one below, and the Tiers list must name the tier buttons in purchase order |
| gate a whole ladder on a factory rung | a row in `Config.TrackUnlock` — `ButtonUnlock` (one purchase gating another across ladders) is gone; never use a cross-track `requires`, which the loader overwrites and the verifier refuses | the reachability fixpoint (it catches gate cycles the `requires` walk cannot) and the gate-names-a-factory-button check |
