...
| `UiKit.lua` | The panel vocabulary: `ROLE` (Config.UI.Palette resolved through Config.UI.Role), `corner`, `stroke`, `panel`, `text`, `button`, `scaleFor`, `safeInsets` — plus `dock` (the four named corners every top-level region is placed with), `control`/`setControlState` (variants and states, from `Config.UI.Button.Variant`), `shade`, `railItem`, and the icon registry — `ICONS`, `icon`, `iconNames`, `fadeIcon` — which `personPlus` folded into. | `HUD`, `SessionUI`, `UpgradeUI` | move to `src/shared` (it would ship to the server build) |
...
| move the belt | `Config.Layout` — `BeltSideX`/`BeltFrontZ`/`BeltBackZ`/`BeltInnerX`/`CollectorAt`/`BeltY`/`BeltWidth`/`BeltSpeed`, or `Config.BeltPaths` for a non-ground path | `verify_config.lua`: legs stay on the plot, the two ground paths mirror, slot distances fit the legs and mirror across their halves, drops-in-flight against the belt capacity model |
