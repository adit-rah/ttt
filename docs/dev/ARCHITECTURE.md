...
| `UiKit.lua` | The panel vocabulary: `ROLE` (Config.UI.Palette resolved through Config.UI.Role), `corner`, `stroke`, `panel`, `text`, `button`, `scaleFor`, `safeInsets` — plus `dock` (the four named corners every top-level region is placed with), `railItem`, and the icon registry — `ICONS`, `icon`, `iconNames`, `fadeIcon` — which `personPlus` folded into. | `HUD`, `SessionUI`, `UpgradeUI` | move to `src/shared` (it would ship to the server build) |
